...
public class GraphValidator {
    ...
    public static void validateXValue(double x) throws ValidationException {
        if (!xValues.contains(x)){
            throw new ValidationException("X must be in set: {-2, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2}");
        }
    }

    public static void validateYValue(double y) throws ValidationException{
        if (y <= -5 || y >=3){
            throw new ValidationException("Y must be in interval (-5, 3)");
        }
    }

    public static void validateRValue(double r) throws ValidationException{
        if (r <= 1 || r >=4){
